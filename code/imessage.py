...
def get_imessages():
    ...
    messages = pd.read_sql_query('''SELECT ROWID, text, handle_id, date
    FROM message T1 
    INNER JOIN chat_message_join T2 
        ON T2.chat_id=25
        AND T1.ROWID=T2.message_id 
    ORDER BY T1.date desc''', conn)
    conn.close()
    return messages
...
def load_database(messages_df):
    with open('/Users/bensolis-cohen/Projects/Chat with my data/data/messages.txt', 'w') as file:
